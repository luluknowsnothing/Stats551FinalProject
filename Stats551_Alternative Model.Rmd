...
# Linear model: 
# Level 1: Others(pi) = Baseline(p) + alpha(p)*Self(pi) + N(0,sd0^2) 
# Level 2: alpha(p) ~ alpha0 + alpha1*Percentile(p) + N(0,sda^2)
 
# Where:
# p is participant index, i is question index 
# Data: Others, Self & Percentile
# parameter: sd0, baseline(p) alpha0, alpha1, sda

# setting priors:
# baseline ~ N(0.25,0.2^2) for every subject p, where mu=1/N guessing level
# alpha0 ~ N(0,0.5^2)
# alpha1 ~ N(0,0.5^2) # our prior assumes everyone has the same slope, alpha(p), no correlation with Percentile 
# sd0,sdb,sda ~ beta(2,5) # We assume a relatively small sd for the error terms, with mode=0.2


# param[1]: baseline(vector), param[2]: alpha0,param[3]: alpha1, param[4]: sd0
...
likelihood = function(param,data,SampleSize){
  ...
  alpha1 = param[2]
  sd0 = param[3]
  baseline = param[4:(SampleSize+3)] # vector
  liklihood_p <- rep(NA,SampleSize)
  for (n in 1:SampleSize){
    data.sub = data[which(data$Subject==n),]
    person_baseline = baseline[n]
    person_alpha = alpha0+alpha1*data.sub$Percentile  # vector sum
    prediction_pi = person_baseline + person_alpha*data.sub$Self # vector sum
    singlelikelihood_pi = dnorm(data.sub$Other, mean = prediction_pi, sd = sd0, log = T) # log=T
    likelihood_p[n] = sum(singlelikelihood_pi) # sum(log) = log(product of p(y))
  }
  sumll = sum(likelihood_p)
}
...
prior= function(param,SampleSize){
  ...
  sd0 = param[3]
  baseline = param[4:(SampleSize+3)] # vector
  baselineprior = dnorm(baseline, mean=0.25, sd=0.2, log = T) # vector
  alpha0prior = dnorm(alpha0, mean=0, sd = 0.5, log = T)
  alpha1prior = dnorm(alpha1, mean=0, sd = 0.5, log = T)
  sdprior = dbeta(sd0, 2, 5, log = T)
  return(alpha0prior+alpha1prior+sdprior+sum(baselineprior)) # sum(log) = log(product of priors)
}
...
proposalfunction = function(param,SampleSize){
    SampleSize = SampleSize
    return(rnorm(3+SampleSize, mean = param, sd= c(0.1,0.1,0.015,rep(0.1,SampleSize))))
}
 
run_metropolis_MCMC = function(startvalue, iterations, data, SampleSize){
    SampleSize = SampleSize
    data=data
    chain = array(dim = c(iterations+1,3+SampleSize))
    chain[1,] = startvalue
    for (i in 1:iterations){
        ...
        if(is.nan(R)){
          R=1 # if chain[i,0] is at a position with 0 density, R is not defined
        }
        if(runif(1) < R){
            chain[i+1,] = proposal
        }else{
...
<<<<<<< HEAD:Stats551_pre-analysis_QY.Rmd
# x <- comb_data[comb_data$Subject == 28, "Self"]
# y <- comb_data[comb_data$Subject == 28, "Other"]
# 
# 
# c <- run_metropolis_MCMC(startvalue, 5000, x, y, 0.8, -0.3, 4)

subj_chain <- function(startvalue, iterations, my_data, alpha, beta, option, n){
  all_chain <- array(dim = c(iterations+1, 3, n))
  
  for (i in 1:n){
    #print(i)
    x <- my_data[my_data$Subject == i, "Self"]
    y <- my_data[my_data$Subject == i, "Other"]
    all_chain[,,i] <- run_metropolis_MCMC(startvalue, iterations, x, y, alpha, beta, option)
  }
  return(all_chain)
}
=======
```
>>>>>>> 193269f20b0ffc6f6ec11cf94b3e506ea4c9aba7:Stats551_Alternative Model.Rmd

```{r}
# run MCMC model for subject 1 as a test
startvalue = c(0.5,0.5,0.1,rep(0.25,SampleSize))
data = comb_data
...
<<<<<<< HEAD:Stats551_pre-analysis_QY.Rmd
for (sub in 1:97){
  acceptanace <- 1 - mean(duplicated(data1_chains[-(1:5000),,sub]))
  acceptance_l[sub] <- acceptanace
}

store_data <- function(startvalue, iterations, comb_data, alpha, beta, option, n){
  out <- array(dim = c(n, 6))
  data_chain <- subj_chain(startvalue, iterations, comb_data, alpha, beta, option, n)
  out[, 1] <- 1:n
  out[, 2] <- apply(data_chain[-(1:5000),2,], 2, mean)
  out[, 3] <- rep(alpha, n)
  out[, 4] <- rep(beta, n)
  bad_sub <- c()
  for (sub in 1:n){
    x <- comb_data[comb_data$Subject == sub, "Self"]
    y <- comb_data[comb_data$Subject == sub, "Other"]
    corr.coe <- cor.test(x,y)
    if (length(unique(x)) == 1 | length(unique(y)) == 1 | corr.coe["estimate"] > 0.999){
      bad_sub <- c(bad_sub, sub)
      out[sub, 5] <- NA
    }else{
      acceptanace <- 1 - mean(duplicated(data_chain[-(1:5000),,sub]))
      out[sub, 5] <- acceptanace
    }
  }
  actual_p <- comb_data[1:n, "Percentile"]
  actual_p[bad_sub] <- NA
  out[, 6] <- rep(sum((out[, 2] - actual_p) ^ 2, na.rm = TRUE), n)
  return(out)
=======
```{r}
# run MCMC model with different hyperparameter combination 
# Goal: find the combination that produces a posterior theta that matches the observed theta the best
# Method: used binary search to approximate the best combination
alpha_test = seq(-1,1,0.05)
beta_test = seq(-1,1,0.05)
Iteration = 10000
comsize= length(alpha_test)*length(beta_test)
Store = data.frame(comsize=1:comsize,
                   hyperparam=I(vector("list",comsize)),
                   chain=I(vector("list", comsize)),
                   acceptance=I(vector("list", comsize)))

#hyperparam_comb = matrix(nrow=comsize,ncol=2)
#acceptance = matrix(nrow=length(alpha_test)*length(beta_test),ncol=SampleSize) # store acceptance rate for each subject and each hyperparam combination
burnIn = 5000
comb_num = 1
startvalue = c(0.25,0.5,0.1)

for (i in 1:length(alpha_test)){
  for (j in 1:length(beta_test)){
    # hyperparam under test
    hyperparam = c(alpha_test[i],beta_test[j])
    # store in hyperparam_comb
    Store$hyperparam[[comb_num]] = hyperparam
    # initialize for each hyperparam combination
    Store$acceptance[[comb_num]] = rep(NA,SampleSize)
    Store$chain[[comb_num]] = vector("list", SampleSize)
    for(n in 1:SampleSize){
      data=comb_data[which(comb_data$Subject==n),]
      tempchain = run_metropolis_MCMC(startvalue, Iteration, data, hyperparam)
      # check acceptance to see whether it is within 20%-30%
      Store$acceptance[[comb_num]][n]=1-mean(duplicated(tempchain[-(1:burnIn),]))
      Store$chain[[comb_num]][[n]]=tempchain
    }
    # update comb_num
    comb_num=comb_num+1
  }
...
test <- store_data(startvalue, 1000, comb_data, 0.5, -0.4, 4, 97)


<<<<<<< HEAD:Stats551_pre-analysis_QY.Rmd
find_hyperp <- function(startvalue, iterations, comb_data, alpha_vec, beta_vec, option, n){
  final_sheet <- array(dim = c(n * length(alpha_vec) * length(beta_vec), 6))
  for (a in 1:length(alpha_vec)){
    print(a)
    alpha <- alpha_vec[a]
    for (b in 1:length(alpha_vec)){
      print(b)
      beta <- beta_vec[b]
      curr_sub <- store_data(startvalue, iterations, comb_data, alpha, beta, option, n)
      final_sheet[((a-1)*n*length(beta_vec)+(b-1)*n+1):((a-1)*n*length(beta_vec)+b*n),] <- curr_sub
    }
  }
  return(final_sheet)
}

alpha_vec <- seq(-1,1,0.1)
beta_vec <- seq(-1,1,0.1)
test2 <- find_hyperp(startvalue, 10000, comb_data, alpha_vec, beta_vec, 4, 97)
=======
Iteration = 10000
chain = vector("list", SampleSize)
acceptance = rep(NA,SampleSize)
burnIn = 5000

for(n in 1:SampleSize){
  tempchain = run_metropolis_MCMC(startvalue, Iteration, data, hyperparam)
  # check acceptance to see whether it is within 20%-30%
  acceptance[n] = 1-mean(duplicated(tempchain[-(1:burnIn),]))
  chain[[n]]=tempchain
}

postPercentile <- sapply(1:SampleSize, function(x) mean(chain[[x]],na.rm=TRUE))
