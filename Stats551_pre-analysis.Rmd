...
O_belief$subid <- c(1:80)
O_belief_l <- reshape(O_belief, varying = list(1:18), direction = "long")

S_belief <- as.data.frame(ChosenBelief)
S_belief_l <- reshape(S_belief, varying = list(1:18), direction = "long")

comb_data <- cbind(S_belief_l, O_belief_l)
comb_data <- comb_data[, -c(3:5)]
names(comb_data) <- c("Q", "Self", "Other", "Subject")
comb_data$Percentile <- rep(SpecificPercentile, 18)
