...
# Likelihood Function
likelihood <- function(x, y, base, alpha, beta, theta, sd){
  pred <- base + x * (alpha + beta * theta)
  ll_vec <- dnorm(y, mean = pred, sd = sd, log = T)
  sum_ll <- sum(ll_vec)
  return(sum_ll)
}

# Prior function
prior <- function(base, theta, sd, option){
  base_mu <- 1/option
  base_prior <- dnorm(base, mean = base_mu, sd = 0.2, log = T)
  theta_prior <- dnorm(theta, mean = 0.65, sd = 0.3, log = T)
  sd_prior <- dbeta(sd, 2, 5, log = T)
  sum_prior <- base_prior + theta_prior + sd_prior
  return(sum_prior)
}

#Posterior function
post <- function(x, y, base, alpha, beta, theta, sd, option){
  return(likelihood(x, y, base, alpha, beta, theta, sd) + prior(base, theta, sd, option))
}

# Metropolis-Hastings MCMC
proposalfunction <- function(param){
  return(rnorm(3, mean = param, sd= c(0.15,0.15,0.015)))
}
 
run_metropolis_MCMC <- function(startvalue, iterations, x, y, alpha, beta, option){
    chain <- array(dim = c(iterations+1, 3))
    chain[1,] <- startvalue
    for (i in 1:iterations){
        proposal <- proposalfunction(chain[i,])
        base_pro <- proposal[1]
        theta_pro <- proposal[2]
        sd_pro <- proposal[3]
        
        if (sd_pro < 0){
          probab <- 0
        }else{
          probab <- exp(post(x, y, base_pro, alpha, beta, theta_pro, sd_pro, option) - 
                        post(x, y, chain[i,1], alpha, beta, chain[i,2], chain[i,3], option))
        }
...
startvalue <- c(0.5, 0.5, 0.5)
data1_chains <- subj_chain(startvalue, 10000, comb_data, 0.8, -0.3, 4, 97)
post_theta <- apply(data1_chains[-(1:5000),2,], 2, mean)
sum_sqd <- sum((post_theta - SpecificPercentile) ^ 2)

acceptance_l <- rep(0,97)
...
## Dataset 2: Abstract Reasoning (Raven's APM) (note: remove 28, 29)
